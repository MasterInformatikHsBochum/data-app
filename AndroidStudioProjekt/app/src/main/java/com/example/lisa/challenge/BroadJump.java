...
public class BroadJump extends MyNavigation {
            ...
            public void onSensorChanged(SensorEvent event) {
                switch (event.sensor.getType()) {
                    ...
                    case Sensor.TYPE_LINEAR_ACCELERATION:
                        aktuelleBeschleunigung = event.values[2];
                        break;
                    //Lichsensor
    ...
    private void setUpIds() {
        start = findViewById(R.id.buttonStartBroadjump);
        cancel = findViewById(R.id.buttonCancelBroadjump);
        result = findViewById(R.id.textResultBroadjump);
        ...
        highscoreText.setText(challange.createHiscoreString(w_score[0],w_score[1],w_score[2],"cm/s"));
    }

    private void actionBroadJump() {

        registerListener(SensorTyp.LIGHT);
        registerListener(SensorTyp.ACCELEROMETER);
        BroadJumpBerechnenThread thread = new BroadJumpBerechnenThread(this, challange);
        thread.execute();
    ...
    public void ueberTrageMessergebnis() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                highScore.new_WS_score(getGemesseneHoechstGeschwindigkeitCmProS());
                //
                // Lade die existierenden Werte aus der Datenbank. Wurden noch keine Werte definiert, wird standardmäßig 0 eingetragen
                float[] w_score = highScore.get_WS_score();
                highscoreText.setText(challange.createHiscoreString(w_score[0],w_score[1],w_score[2],"cm/s"));
    ...
    public Float getGemesseneHoechstGeschwindigkeitMProS(){
        return gemesseneHoechstGeschwindigkeitMProS;
    }
